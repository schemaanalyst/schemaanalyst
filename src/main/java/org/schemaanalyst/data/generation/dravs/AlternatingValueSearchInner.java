...
public class AlternatingValueSearchInner extends SearchMini<Data> {
	...
	public AlternatingValueSearchInner(Random random, CellInitializer startInitializer,
	...
	/*
	 * @Override public void search(Data data, PredicateChecker checker,
	 * List<Cell> cells) { // set up this.subsetData = data; //cells =
	 * data.getCells(); this.cells = cells;
	 * 
	 * // start // startInitialiser.initialize(data); lastObjVal = null;
	 * evaluate();
	 * 
	 * 
	 ...
	 * 
	 * // main loop while (!terminationCriterion.satisfied()) {
	 * 
	 * alternateThroughValues();
	 * 
	 * if (checker.check()) { break; }
	 * 
	 * if (!terminationCriterion.satisfied()) { restartsCounter.increment(); }
	 * 
	 * if (!terminationCriterion.satisfied()) {
	 * restartInitialiser.initialize(subsetData); lastObjVal = null; evaluate();
	 * } } }
	 * 
	 * @Override public void search(Data data, PredicateChecker checker, Cell
	 * cell) { // set up this.subsetData = data; //cells = data.getCells();
	 * this.cells.add(cell);
	 * 
	 * // start // startInitialiser.initialize(data); lastObjVal = null;
	 * evaluate();
