		...
		expression.accept(new ExpressionAdapter() {
			...
			void setNonCompliant(Expression expression) {
				...
				for (Column column : columnsInvolved) {
					nonComplyingData.addRow(row.getTable(), new Row(row.getCell(column)));
			...
			public void visit(BetweenExpression expression) {
				...
				boolean lhsResult = new RelationalChecker(subject, RelationalOperator.GREATER_OR_EQUALS, lhs, allowNull)
						.check();

				boolean rhsResult = new RelationalChecker(subject, RelationalOperator.LESS_OR_EQUALS, rhs, allowNull)
						.check();
			...
			public void visit(InExpression expression) {
				...
				for (Value rhsValue : rhsValues) {
					boolean valueResult = new RelationalChecker(lhs, RelationalOperator.EQUALS, rhsValue, allowNull)
							.check();
					if (valueResult) {
						result = true;
					}
