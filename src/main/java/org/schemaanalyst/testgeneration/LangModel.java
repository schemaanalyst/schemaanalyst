...
public class LangModel {
	...
	public LangModel(String lmFileName) throws IOException {
		...
		while ((strLine = br.readLine()) != null) {
			...
			if (match_ngram_len.find()) {
				...
				if (match_unigram.find()) {
					...
					unigram_probs.put(unigram, new Double(unigram_prob));
					unigram_backoff_probs.put(unigram, new Double(unigram_backoff_prob));
				...
				if (match_bigram.find()) {
					...
					bigram_probs.put(bigram, new Double(bigram_prob));
		...
		// probability)
		String whitespace = " ";
		unigram_probs.put(whitespace, new Double(highest_unigram_prob));
		unigram_backoff_probs.put(whitespace, new Double(highest_unigram_prob));
