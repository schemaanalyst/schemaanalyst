...
public class SearchConstraintCoverer extends DataGenerator {
	...
	public SearchConstraintCoverer(Search<Data> search, Schema schema,
			DBMS dbms, int satisfyRows, int negateRows) {
		this.search = search;
		this.schema = schema;
		this.dbms = dbms;
	...
	protected SearchConstraintGoalReport generateData(Table constraintTable,
			Constraint constraint, List<Table> tables, int numRows) {
		...
		search.setObjectiveFunction(new SchemaConstraintSystemObjectiveFunction(
				schema, state, constraintTable, constraint));
		SearchConstraintGoalReport goalReport = performSearch(constraintTable,
				constraint, data);
	...
	protected SearchConstraintGoalReport performSearch(Table table,
			Constraint constraint, Data data) {
		// create the report and start the clock ...
		SearchConstraintGoalReport goalReport = new SearchConstraintGoalReport(
